...
from brainstorm.core.services.model_service import create_model, get_model, get_models, update_model, delete_model
...
async def create_model(
    ...
    """
    try:
        db_model = await create_model(db, model_data)
        return db_model
...
async def get_models(
    ...
    """
    models = await get_models(db, skip, limit)
...
async def update_model(
    ...
    """
    try:
        db_model = await update_model(db, model_id, model_data)
        if not db_model:
            raise HTTPException(status_code=404, detail=f"Model with ID {model_id} not found")
        return db_model
    except Exception as e:
